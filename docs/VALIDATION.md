...
Before first-alpha release acceptance, commit reproducible npm and Cargo lockfiles generated by a toolchain-capable environment and verify that a clean checkout can install/build from them.

Until lockfiles exist, a hosted job using unlocked dependency resolution is not a reproducible required gate.
