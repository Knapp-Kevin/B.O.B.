...
function renderInbox() {
  ...
  return `${header("Inbox", "Capture first. Decide what it is later.")}${capture()}<section class="inbox-grid"><article class="panel inbox-list"><div class="filters">${kinds.map((kind) => `<button class="${state.filter === kind ? "active" : ""}" data-filter="${kind}">${kind === "all" ? "All items" : `${kind}s`}</button>`).join("")}</div>${visible.length ? visible.map((item) => `<div class="item"><i class="kind ${item.kind}">${icon(item.kind)}</i><div><small>${item.kind.toUpperCase()}</small><strong>${escapeHtml(item.title)}</strong><p>${item.due ? `<span>${escapeHtml(item.due)}</span>` : ""}${item.estimate ? `<span>${item.estimate} min</span>` : ""}<span class="${item.priority}">${item.priority}</span></p></div>${item.kind === "task" ? `<button title="Make next action" aria-label="Make ${escapeHtml(item.title)} the next action" data-active="${item.id}">›</button>` : `<span aria-hidden="true"></span>`}</div>`).join("") : `<div class="empty"><b>Nothing waiting in this view.</b><span>Capture first. B.O.B. can help organize later.</span></div>`}</article><aside data-secondary><article class="panel side"><small>INBOX SNAPSHOT</small><h3>${visible.length} things waiting</h3><p><span>Actionable tasks</span><b>${visible.filter((item) => item.kind === "task").length}</b></p><p><span>Needs a decision</span><b>${visible.filter((item) => item.kind !== "task").length}</b></p><button class="primary" id="organize" ${visible.some((item) => item.kind === "task") ? "" : "disabled"}>Organize with B.O.B.</button></article><article class="panel side"><small>CAPTURE RULE</small><h3>Do not make yourself classify it yet.</h3><p>Get it out of your head first. Structure can come later.</p></article></aside></section>`;
}
