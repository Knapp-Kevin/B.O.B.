...
function renderToday() {
  ...
  const next = focus[0] ?? activeItem();
  const hasPlan = focus.length > 0;
  ...
  const planRows = focus.length
    ...
    : `<div class="empty"><b>No remaining planned work.</b><span>Capture something or promote an Inbox task when you are ready.</span></div>`;

  return `${header("Today", "Let’s make the next move obvious.")}
  <section class="grid top-grid">
    <article class="panel next"><small>NEXT ACTION</small><div class="next-body"><span class="star">★</span><div>${hasPlan ? `<h2>${escapeHtml(next.title)}</h2><p>${next.estimate ? `~${next.estimate} min` : "Small step"} · ${next.priority === "high" ? "High impact" : "Useful progress"}</p><div class="actions"><button class="primary" id="start" ${next.status === "doing" ? "disabled" : ""}>${next.status === "doing" ? "In progress" : "Do it now"}</button><button class="plain" id="defer">◷ Not now</button></div>` : `<h2>Nothing is demanding attention.</h2><p>Your remaining plan is clear. Capture something new only when it actually matters.</p>`}</div></div></article>
    <article class="panel focus" data-secondary><div class="panel-title"><small>WHAT MATTERS TODAY</small><span>Up to three</span></div>${focus.length ? focus.map((item, index) => `<button data-complete="${item.id}" aria-pressed="${item.status === "done"}"><i>${index + 1}</i><strong class="${item.status === "done" ? "done" : ""}">${escapeHtml(item.title)}</strong><span>${item.estimate ?? ""}${item.estimate ? " min" : ""}</span><em>${item.status === "done" ? "✓" : ""}</em></button>`).join("") : `<div class="empty"><b>No focus items remain.</b><span>B.O.B. will not invent busywork to fill the card.</span></div>`}</article>
  </section>
  ${capture()}
  <section class="grid bottom-grid" data-secondary>
    <article class="panel day"><div class="panel-title"><div><small>REMAINING PLAN</small><h3>${focus.length ? `${focus.length} focus item${focus.length === 1 ? "" : "s"}${totalEstimated ? ` · ~${totalEstimated} min estimated` : ""}` : "No active plan"}</h3></div><button class="ghost small" id="replan" ${focus.length ? "" : "disabled"}>Replan</button></div>${planRows}<p class="fixture-note">This order comes from B.O.B.’s deterministic planner. Clock-time blocks will appear only after fixed commitments and availability have typed scheduling data.</p></article>
    <div class="stack"><article class="panel resume"><small>RESUME WHERE I LEFT OFF</small>${recent.length ? recent.map((item) => `<button data-active="${item.id}"><span>↗</span><strong>${escapeHtml(item.title)}</strong><b>›</b></button>`).join("") : `<div class="empty"><span>No other unfinished work is competing for attention.</span></div>`}</article><article class="encourage"><div><h3>You’ve got this.</h3><p>Progress beats perfection. B.O.B. keeps the rest recoverable.</p></div><img src="/bob-icon.png" alt=""></article></div>
