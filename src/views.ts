...
function handoffCard() {
  ...
  return `<article class="handoff-card"><div class="panel-title"><small>SESSION HANDOFF PREVIEW</small><button class="ghost small" id="clear-handoff">Clear</button></div><dl><div><dt>Objective</dt><dd>${escapeHtml(state.handoff.objective)}</dd></div><div><dt>State</dt><dd>${escapeHtml(state.handoff.state)}</dd></div><div><dt>Next</dt><dd>${escapeHtml(state.handoff.next)}</dd></div></dl><p>Stored in B.O.B.’s local canonical state so the objective and next move can survive a full restart.</p></article>`;
}
